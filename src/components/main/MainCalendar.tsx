...
export default function Calendar() {
  ...
  const dayOffStartDate = dayOffs.map(x =>
    moment(x['startDate']).format('YYYYMMDD')
  );
  const dayOffEndDate = dayOffs.map(x =>
    moment(x['endDate']).format('YYYYMMDD')
  );
  ...
  const getDateRange = () => {
    ...
    for (let i = 0; i < dayOffStartDate.length; i++) {
      ...
      const filteredDaysInRange = (daysInRange as string[]).filter(
        date => date !== end.format('YYYYMMDD')
      );
  ...
  const Weeks = () => {
    ...
    for (let i = 0; i < 7; i++) {
      days.push(
        <div
          className="grow border-r-2 last:border-0 bg-mainGray pl-1.5"
          key={i}>
          {date[i]}
        </div>
      );
  ...
  const RenderDays = () => {
    ...
    while (day <= endDate) {
          ...
          i === 0 ||
            ...
            <div
              ...
              {checkDayOffS.length > 0 && checkDayOffE.length === 0 ? (
                <div className="w-full h-5 mt-3 bg-blue-500"></div>
              ) : null}
            ...
            <div
              ...
              key={formattedDate}>
              {checkedMonth === currentMonth ? (
                <div className="w-6 h-6 pt-1.5 pl-1.5">{formattedDate}</div>
              ) : (
                <div className="w-6 h-6 pt-1.5 pl-1.5 text-mainGray">
                  {formattedDate}
                </div>
      ...
      rows.push(
        <div className="flex" key={formattedDate}>
          {days}
        </div>
      );
      days = [];
      ...
      <div>
        ...
        <div className="flex h-20 text-3xl">
          <ChevronLeftIcon
            ...
            onClick={prevMonth}
          />
          <div className="text-center grow top-0 bottom-0 m-auto">
